<template>
  <div>
      <div class="form-row">
    <b-form  class="form-group col-md-4" @submit="onSubmit" @reset="onReset" v-if="show">
      <b-form-group
        id="input-group-1"
        label="Summa:"
        label-for="input-1"
...
<b-form-group id="input-group-2" label="Valuta:" label-for="input-2">
      ...
      <b-form-group id="input-group-3" label="Kryptovaluta:" label-for="input-3">
        <b-form-select
          id="input-3"
          v-model="valuta2"
...
watch: {
      summa(nyaSumma, gamlaSumma) {
        console.log(`Användaren har ändrat summan ifrån ${gamlaSumma} till ${nyaSumma}`)
      }
    },
    data() {
      return {
